    ...
    <div className="fixed inset-0 min-h-screen bg-[#050505] flex flex-col items-center justify-center px-6 overflow-hidden z-[9999] select-none">
      ...
      <div className="animate-float mb-16 relative z-10 text-center">
        <div className="relative inline-block">
          ...
          >
            <span className="text-[#00ffff]">GHC</span>
            <span className="text-white">AREERS</span>
          </h1>
          {/* Linha Neon */}
          <div className="absolute -bottom-4 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[#00ffff] to-transparent shadow-[0_0_15px_#00ffff]" />
        </div>
      </div>
      ...
      <a
        ...
        className="group relative inline-flex items-center gap-3 px-10 py-5 bg-[#00ffff] text-black font-black text-xl rounded-full transition-all duration-300 hover:scale-110 animate-pulse-glow z-10"
      >
        <MessageCircle className="w-7 h-7 fill-current" />
        <span>Entrar no Canal</span>
        
        {/* Hover ring effect */}
        <span className="absolute inset-0 rounded-full border-4 border-[#00ffff] opacity-0 group-hover:opacity-100 group-hover:scale-125 transition-all duration-500" />
      </a>

      {/* Bottom decoration */}
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-4 opacity-50">
        <div className="w-3 h-3 rounded-none rotate-45 border border-[#00ffff] bg-[#00ffff] animate-pulse" />
        <div className="w-3 h-3 rounded-none rotate-45 border border-white" />
        <div className="w-3 h-3 rounded-none rotate-45 border border-[#00ffff] bg-[#00ffff] animate-pulse" style={{ animationDelay: '0.5s' }} />
      </div>
    </div>
  );
};
